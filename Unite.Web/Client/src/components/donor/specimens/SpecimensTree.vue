       ...
       <div class="row q-gutter-x-xs">
          ...
          <div :class="{ 'text-subtitle1 text-bold' : prop.node.active} ">
            <span>
             ...
             - {{getName(prop.node)}}
            <q-badge v-if="prop.node.active" align="top">this donor</q-badge>
          </div>
       </div>
      </template>
      ...
      <template v-slot:default-body="prop">
        <div class="row">
          <q-card bordered :flat="!prop.node.active">
            <q-card-section class="q-pa-sm" v-if="!prop.node.active">
              <div class="row q-gutter-x-sm" :class="text-caption">
                <template v-if="!!prop.node.tissue">
                  <div>
                    <span>Type: </span>
...
export default {
  ...
  methods: {
    ...
    buildNode(specimen, current) {
      var node = { id: specimen.id, active: specimen.id == current };
      
      if (!!specimen.tissue) {
        node.key = `tissue.${specimen.id}`;
        node.tissue = specimen.tissue;
      } else if (!!specimen.cellLine) {
        node.key = `cell.${specimen.id}`;
