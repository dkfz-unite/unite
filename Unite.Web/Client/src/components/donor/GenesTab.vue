...
export default {
  ...
  data() {
    return {
      ...
      criteria: this.$store.state.donor.genesSearchCriteria
    };
  },
  ...
  watch: {
    ...
    rowsSelected(value) {
      this.$store.state.donor.genesSelected = value;
    }
  },
  ...
  methods: {
    ...
    async fetchData() {
      try {
        this.loading = true;
        let data = await apiClient.getGenes(this.donor.id, this.criteria);
        this.rows = data ? data.rows : [];
        this.rowsTotal = data ? data.total : 0;
        this.rowsSelected = this.$store.state.donor.genesSelected;
      } catch {
        this.rows = [];
        this.rowsTotal = 0;
        this.rowsSelected = [];
