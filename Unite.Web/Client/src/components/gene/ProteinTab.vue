...
export default {
  ...
  computed: {
    ...
    plotData() {
      if (!this.transcript || !this.gene || !this.data) {
        return null;
      } else {
        return dataService.prepareData(this.transcript, { id: null }, this.data);
      }
    }
  },
  ...
  methods: {
    async fetchTranscripts() {
      try {
        ...
        this.transcripts = await apiClient.getGeneTranscripts(this.gene.id);
