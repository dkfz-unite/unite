<template>
  <div class="col q-gutter-y-sm">
    <div class="row">
      <span class="text-h5 u-text-title">MR Images</span>
    </div>
...
import UDataTable from "@/domain/images/mrs/components/Table.vue";
import tabTableMixin from "@/domain/_shared/entry/components/tab-table-mixin";
import filters from "@/domain/images/mrs/models/filters/mr-filters";
...
export default {
  ...
  data() {
    return {
      domain: this.$store.state.donor.mrs,
      model: "mr",
      filters: filters
    }
  },
  ...
  methods: {
    async fetchData(searchCriteria) {
      return await api.searchImages(this.donor.id, "mr", searchCriteria);
    }
  }
}
</script>